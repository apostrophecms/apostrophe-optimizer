...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    if (optimizer.options.stats) {
      ...
      if (!optimizer.statsTimeout) {
        optimizer.statsTimeout = setTimeout(function() {
          self.apos.utils.debug(optimizer.stats);
          optimizer.statsTimeout = false;
        }, 2000);
      }
    }
    ...
    self.optimize = function(results, callback) {
      ...
      return Promise.try(function() {
        ...
        if (optimizer.options.debug) {
          optimizer.debug('**** now our ids are:', JSON.stringify(_.keys(req.optimizeDocs), null, '  '));
        }
